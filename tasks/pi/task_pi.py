...
class TaskPI(Task):
    def __init__(self):
        with open("prompts/pi/pi_full_prompt.txt", "r") as f:
            self.fully_specified_prompt = f.read()
        with open("prompts/pi/pi_system_prompt.txt", "r") as f:
    ...
    def get_dataset_file(self) -> str:
        return "data/pi/sharded_pi.json"
    ...
    def get_answer_description(self) -> str:
        return "The answer should be a single letter (A or B)."
    ...
    def evaluator_function(self, extracted_answer: str, sample: Dict[str, Any]) -> bool:
        # Ground truth answer (expected to be 'A' or 'B')
        gold = sample["label"].strip().upper()

        try:
            # Extract only the first valid 'A' or 'B' from the model output
            # e.g., "The correct answer is A." -> "A"
            match = re.search(r"\b([AaBb])\b", extracted_answer)
            if not match:
    ...
    def populate_fully_specific_prompt(self, sample: Dict[str, Any]) -> str:
        """
        Populate the full PI prompt with sentence1 and sentence2.
        Keeps [[fewshot_examples]] placeholder intact for later insertion.
        """
        return (
            self.fully_specified_prompt
            .replace("[[sentence1]]", sample["sentence1"])
            .replace("[[sentence2]]", sample["sentence2"])
        )
...
if __name__ == "__main__":
    ...
    #     "b",
    #     "Answer: a",
    #     "The answer is C",  # invalid
    #     "",
    # ]

    # for sample in samples:
    #     for output in model_outputs:
    #         result = task.evaluator_function(output, sample)
    #         print(f"Gold={sample['answer']!r}, Output={output!r}")
    #         pprint.pprint(result)
    #         print("-" * 40)

    # Example sample for testing
    sample = {
        "task_id": "sharded-pi/255",
        "sentence1": "Noong 1947, nahati ang West Punjab ng British India ayon sa hangganang pangrelihiyoso sa Punjab Province at East Punjab.",
        "sentence2": "Noong 1947 nahati ang lalawigan ng Punjab sa British-India ayon sa hangganang pangrelihiyoso sa West Punjab at East Punjab.",
        "label": "B",
        "shards": [
            {"shard_id": 1, "shard": "paraprase ba ang SENTENCE_1 ng SENTENCE_2?"},
            {"shard_id": 2, "shard": "Ayon sa SENTENCE_1 ang paghati ay nangyari noong 1947."},
            {"shard_id": 3, "shard": "Sinabi sa SENTENCE_1 na nahati ang West Punjab ng British India."},
            {"shard_id": 4, "shard": "Ayon sa SENTENCE_1, ang paghati ay ayon sa hangganang relihiyoso."},
            {"shard_id": 5, "shard": "Sa SENTENCE_1, ang hangganang relihiyoso ay sa pagitan ng Punjab Province at East Punjab."},
            {"shard_id": 6, "shard": "Ayon sa SENTENCE_2 ang paghati ay nangyari noong 1947."},
            {"shard_id": 7, "shard": "Batay sa SENTENCE_2, nahati ang lalawigan ng Punjab sa British-India."},
            {"shard_id": 8, "shard": "Ayon sa SENTENCE_2, ang paghati ay ayon sa hangganang relihiyoso."},
            {"shard_id": 9, "shard": "Sa SENTENCE_2, ang hangganang relihiyoso ay sa pagitan ng West Punjab at East Punjab."}
        ],
        "task": "pi"
    }
