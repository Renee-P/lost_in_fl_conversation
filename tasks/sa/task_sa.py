...
class TaskSA(Task):
    def __init__(self):
        with open("prompts/sa/sa_full_prompt.txt", "r") as f:
            self.fully_specified_prompt = f.read()
        with open("prompts/sa/sa_system_prompt.txt", "r") as f:
    ...
    def get_dataset_file(self) -> str:
        return "data/sa/sharded_sa.json"
    ...
    def get_answer_description(self) -> str:
        return "The answer should be 'positibo', 'negatibo', or 'neutral'."
    ...
    def evaluator_function(self, extracted_answer: str, sample: Dict[str, Any]) -> bool:
        # Ground truth answer (expected to be 'positibo', 'negatibo', or 'neutral')
        gold = sample["label"].strip().lower()

        try:
            # Extract only the first valid 'positibo', 'negatibo', or 'neutral' from the model output
            # e.g., "Ang sagot ay positibo." -> "positibo"
            match = re.search(r"\b(positibo|negatibo|neutral)\b", extracted_answer, re.IGNORECASE)
            if not match:
...
if __name__ == "__main__":
    sample = {
        "task_id": "sharded-sa/1",
        "text": "Yung akala mo imported biscuits na pag open mo may kamatis na puro karayom :(",
        "shards": [
            {
                "shard_id": 1,
                "shard": "sentimyento sa pangungusap?"
            },
            {
                "shard_id": 2,
                "shard": "Unang sinabi ay 'Yung akala mo imported biscuits'."
            },
            {
                "shard_id": 3,
                "shard": "Pag open nya ang laman pala ay kamatis."
            },
            {
                "shard_id": 4,
                "shard": "ang kamatis ay 'puro karayom :('"
            }
        ],
        "task": "sa"
    }
