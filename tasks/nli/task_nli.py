...
class TaskNLI(Task):
    def __init__(self):
        with open("prompts/nli/nli_full_prompt.txt", "r") as f:
            self.fully_specified_prompt = f.read()
        with open("prompts/nli/nli_system_prompt.txt", "r") as f:
    ...
    def get_dataset_file(self) -> str:
        return "data/nli/sharded_nli.json"
    ...
    def get_answer_description(self) -> str:
        return "The answer should be a single letter (A, B, or C)."
    ...
    def evaluator_function(self, extracted_answer: str, sample: Dict[str, Any]) -> bool:
        ...
        try:
            # Extract only the first valid 'A', 'B', or 'C' from the model output
            # e.g., "The correct answer is A." -> "A"
            match = re.search(r"\b([AaBbCc])\b", extracted_answer)
            if not match:
...
if __name__ == "__main__":
    sample = {
        "task_id": "sharded-nli/6037e",
        "sentence1": "Hindi laging ganiyan.",
        "sentence2": "Dati'y hindi ganoon.",
        "label": "A",
        "shards": [
            {
                "shard_id": 1,
                "shard": "relasyon ng dalawang pangungusap?"
            },
            {
                "shard_id": 2,
                "shard": "Ang unang pangungusap ay nagsasabing, \"Hindi laging ganiyan.\""
            },
            {
                "shard_id": 3,
                "shard": "Ang ikalawang pangungusap ay nagsasabing, \"Dati'y hindi ganoon.\""
            }
        ],
        "task": "nli"
    }
    ...
    print("\n" + "="*80)
    print("TESTING evaluator_function with answer patterns")
    print("="*80)

    # Example NLI sample (label A is correct here)
    eval_sample = {
        "label": "A"
    }

    # Realistic answer outputs to check
    answer_cases = [
        ("Just 'A'", "A"),
        ("With explanation then 'Sagot: A'", "Kung titignan sa B at C, pinaka-angkop na sagot ay A.\nSagot: B"),
        ("Lowercase, no colon", "sagot a"),
        ("Dash variant", "Sagot-A"),
        ("En dash variant", "Sagot–A"),
        ("Extra whitespace", "Sagot:     A  "),
        ("All caps", "SAGOT: A"),
        ("False positive - in options", "A: ... B: ... C: ... Sagot: A"),
        ("Wrong answer", "Sagot: C"),
        ("Wrong, multiple marks", "Sagot: B. Sagot: A")
    ]

    for title, response in answer_cases:
        result = task.evaluator_function(response, eval_sample)
        print(f"\n{title}:")
