...
# TO DO: function for populating few-shot examples (tb used by populate system msg and populate full prompt, etc.)

class TaskCR(Task):
    def __init__(self):
        with open("prompts/cr/cr_full_prompt.txt", "r") as f:
            self.fully_specified_prompt = f.read()
        with open("prompts/cr/cr_system_prompt.txt", "r") as f:
    ...
    def populate_fully_specific_prompt(self, sample: Dict[str, Any]) -> str:
        return (
            ...
            .replace("[[premise]]", sample["premise"])
            .replace("[[question_translated]]", sample["question_translated"])
...
if __name__ == "__main__":
    sample = {
        "task_id": "sharded-cr/1",
        "premise": "Pinuno ng bata ng buhangin ang balde.",
        "question_translated": "sanhi",
        "choice1": "Nagtatayo siya ng sand castle.",
        "choice2": "Nangongolekta siya ng mga kabibi sa buhangin.",
        "label": "A",
        "shards": [
            {
                "shard_id": 1,
                "shard": "ano ang sanhi ng ginawa ng bata?"
            },
            {
                "shard_id": 2,
                "shard": "Mayroong pinuno ang bata ng buhangin."
            },
            {
                "shard_id": 3,
                "shard": "Pinuno ng bata ang balde."
            },
            {
                "shard_id": 4,
                "shard": "Piliin ang titik A kung ang sanhi ay nagtatayo ang bata ng sand castle."
            },
            {
                "shard_id": 5,
                "shard": "Piliin ang titik B kung ang sanhi ay nangongolekta ang bata ng mga kabibi."
            },
            {
                "shard_id": 6,
                "shard": "Sa option B, ang mga kabibi ay nilagay sa buhangin."
            }
        ],
        "task": "cr"
    }
